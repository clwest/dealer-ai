...
> blast radius is **6 endpoints** (3× understatement). Every
> M27+ §5.a selection made from the current audit inherits
> that drift.
>
...
2. What is the true M25-close-shipped coverage baseline
   after the parser fix? (Expected: **120 / 154**,
   confirmed only after the regenerated artifact and
   direct repository inspection agree per §5.d.)
...
- `docs/roadmap/M21_OPERATIONAL_SURFACE_AUDIT.md` —
  regenerated by the fixed script. Six rows flip from
  `defer-candidate-O2` → `covered`. Coverage summary
  updates from **114 / 154 → 120 / 154**. Row wrapper-
  ordering diffs (like the SESSION_189 §3 regen's
...
- **Audit output format changes.** Markdown row shape,
  ...
  strictly: (a) 6 rows flip `defer-candidate-O2` →
  `covered` with wrapper columns populated, (b)
  coverage summary numerator updates 114 → 120.
...
**Positive regression cases (one per confirmed false
positive from SESSION_189 §3 tracing):**
...
6. `` `/vehicles/${vehicleId}/${qs ? `?${qs}` : ""}` ``
   → analogous, wrapper `getVehicleDetail`. This case
   also verifies the tokenizer handles **two** sequential
   `${...}` interpolations where only the second is
   nested-template-bearing.
...
case 6, the two-interpolation case) must assert that
the resolved GET-wrapper is *not* incorrectly claimed
against an unrelated POST endpoint at the same URL
prefix. Guards against silent regression of the M23.1
verb filter.

**Test count target: 12 positive + negative test
methods** (6 positive + 6 negative). Backend baseline
expected to move **4,793 → 4,805** at M26.1 close
(±1–2 depending on test grouping).
...
- Row 5 (`vehicles/<int:vehicle_id>/`) — Frontend
  consumers column populated with `api.ts:626
  getVehicleDetail`; disposition flips → `covered`.
...
all six):** For each of the six reclassified rows,
open the wrapper's source file at the reported
`{filename}:{line}` and confirm:
...
The corrected coverage baseline of **120 / 154** is
recorded in `docs/CAPABILITY_MATRIX.md` §7α and
elsewhere ONLY after both of the following agree:

1. **Regenerated artifact.** The refreshed
   `M21_OPERATIONAL_SURFACE_AUDIT.md` coverage summary
   reads `114 → 120`.
2. **Direct repository inspection.** All six §5.d
   Phase 2 manual verifications pass.

If either source disagrees, the baseline is NOT
updated. The M25.3 handoff's 2-endpoint understatement
(corrected to 6 at SESSION_189 §3) is precisely the
...
3. Add `backend/dealer_ai/tests/test_audit_operational_
   surface.py` with 6 positive + 6 negative regression
   tests per §5.c.
...
6. Diff the regenerated artifact against the pre-fix
   version. Assert exactly the six §5.d Phase 1
   expected reclassifications appear, no more, no
   fewer.
7. Perform §5.d Phase 2 manual verification on each of
   the six rows. If any fails: halt, document, treat as
...
11. Overwrite `00-START-NEXT-SESSION.md` with SESSION_191
    ...
    H remains separate; corrected 120 / 154 baseline).
...
- ❌ Do NOT change any endpoint disposition beyond the
  ...
  produces. `recommend_disposition()` heuristic is
  out of scope per §3.
